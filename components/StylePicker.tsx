...
export default function StylePicker({ selected, onChange, customPrompt, onCustomPromptChange, disabled }: StylePickerProps) {
  return (
    <div className="w-full mb-6">
      <p className="text-sm text-white/30 mb-3 text-center">Choose a style</p>
        ...
        {STYLE_LIST.map((style) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              className={`relative flex flex-col items-center p-2 sm:p-3 rounded-xl border backdrop-blur-sm transition-all ${
                isSelected
            ...
            >
              <div
                className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-gradient-to-br ${style.gradient} flex items-center justify-center text-white mb-1.5`}
              >
                {ICONS[style.icon]}
              </div>
              <span className={`text-[10px] sm:text-xs font-medium truncate w-full text-center ${isSelected ? "text-white" : "text-white/60"}`}>
                {style.name}
              </span>
            </button>
          );
      ...
      {selected === "custom" && (
        <div className="mt-4 flex gap-2">
          <input
            ...
            disabled={disabled}
            className="flex-1 px-4 py-3 text-sm bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-xl focus:ring-1 focus:ring-white/30 focus:border-white/20 disabled:opacity-50 disabled:cursor-not-allowed text-white placeholder-white/30 outline-none transition-all"
            maxLength={MAX_CUSTOM_STYLE_LENGTH}
          />
          <button
            type="button"
            onClick={() => onCustomPromptChange(generateRandomStyle())}
            disabled={disabled}
            className="px-3 py-3 bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-xl hover:bg-white/[0.08] hover:border-white/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all group"
            title="Random style"
          >
